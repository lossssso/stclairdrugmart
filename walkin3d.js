/* Store walk-in 3D — the welcome section's storefront photo comes alive.
   As you scroll, the real photo card grows to fill the screen, slowly
   dissolves into a stylized 3D rendition of the store (Three.js), and the
   camera walks you in through the front door to clickable chat bubbles.
   Progressive enhancement: the section stays display:none unless this
   module activates it (html.w3d-on). No-JS, reduced-motion, weak devices
   and WebGL-less browsers keep the classic page. Three.js is vendored at
   vendor/three.module.min.js and only imported once the user nears the
   section. */
...
(function () {
  ...
  /* captured at load — the site's URL path-sync rewrites the hash away on
     scroll. #w3ddebug disables the slow-frame bail-out (testing only). */
  ...
  var PHOTO_RATIO = 1281 / 1227; /* storefront_exterior.webp h/w */
  ...
  function buildEngine(THREE, gsap, ScrollTrigger) {
    ...
    function frame(now) {
      ...
      /* welcome photo → stylized store morph: the card grows to full
         bleed (0→.10), holds, then dissolves into the 3D scene (.13→.26)
         with a slow continuation zoom so the photo "turns into" it */
      if (photoCard) {
        if (p < 0.3) {
          var g = seg(p, 0, 0.10);
          var cw = Math.min(w * (w < 700 ? 0.86 : 0.62), 660);
          var ch = cw * PHOTO_RATIO;
          if (ch > h * 0.74) { ch = h * 0.74; cw = ch / PHOTO_RATIO; }
          photoCard.style.display = 'block';
          photoCard.style.left = ((w - cw) / 2 * (1 - g)).toFixed(1) + 'px';
          photoCard.style.top = ((h - ch) / 2 * (1 - g)).toFixed(1) + 'px';
          photoCard.style.width = (cw + (w - cw) * g).toFixed(1) + 'px';
          photoCard.style.height = (ch + (h - ch) * g).toFixed(1) + 'px';
          photoCard.style.borderRadius = (18 * (1 - g)).toFixed(1) + 'px';
          photoCard.style.opacity = (1 - seg(p, 0.13, 0.26)).toFixed(3);
          if (photoImg) photoImg.style.transform =
            'scale(' + (1 + 0.12 * seg(p, 0.08, 0.26)).toFixed(4) + ')';
        } else {
          photoCard.style.display = 'none';
        }
      }

      /* 3D scene fades in under the photo as the scroll begins, so at rest
         the photo card floats alone on the sky gradient */
      canvas.style.opacity = seg(p, 0.02, 0.1).toFixed(3);
      ...
      /* "what brings you in?" bubble greets you once you're inside */
      if (qBubble) {
